...
;(function (window, undefined) {
    ...
    $.fn.tableExport = function (options) {
        ...
        //Adding defaults values to be used by the plugin.
        /*settings.defaults = {
            exclude: ".noExl",
            name: "TableExcel"
        };*/
        ...
        return this.each(function () {
            var $el = $(this),
                $rows = settings.headings ? $el.find('tr').not(settings.exclude) : $el.find('tr:has(td)').not(settings.exclude), // Write code for footer if needed
                ...
                exporters = {
                    xlsx: function (rDel, name) {
                        var dataURL = $rows.map(function (i, val) {
                                var $cols = $(val).find('th, td').not(settings.exclude);
                                return $cols.map(function (i, val) {
                                    if($(val).hasClass('amount') || $(val).children().hasClass('amount')){
                                        return parseFloat($(val).text().replace(/[$,]/g, "").trim());
                                    }
                                    return $(val).text().trim();
                                });
                            }).get(),
                            dataURLmodified = [];
                        $(dataURL).each(function(i,val){val.prevObject={};val.context={};dataURLmodified.push(val);});
                        var dataObject = JSON.stringify({
                                data: dataURLmodified,
                                name: name
                            }),
                            myFile = name + ".xlsx",
            ...
            function createObjButton(dataObject, myFile, myContent, myClass) {
                //Button will not be displayed on the screen. We will just call button click.
                ...
                if(dataObject){
                    ...
                    export2xlsx(data, fileName, settings.context);
                } else {
                    return;
                }
            }
    ...
    $.fn.tableExport.defaults = {
        //Add code for footer below if needed
        //footer:true,
...
function export2xlsx(data, name, context) {
    var wb = new Workbook(),
        ws = createSheet(data);
